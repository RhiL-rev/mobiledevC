## TODOLISTをつくる  
>Problem  
   tasksに追加した後、setState()を実行しているが、その後ウィンドウサイズなどをちょっと変更しないとtasksが表示されない。  
...
そのなかでCheckTileクラスを作成し、その中でboolを用意し、CheckBoxListTileのisCheckedに設定する。→タップするとチェックが入る  

TextStyleを2つ作成し、isCheckedで切り替えることで、チェックの状態に応じてTextDecoration.lineThroughの有無と文字色を変更できる。
